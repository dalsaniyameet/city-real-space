...
// DevTools open detection — blur karo page
(function() {
  var devOpen = false;
  var threshold = 160;
...
function dbToCard(p) {
  ...
  const badgeClassMap = { 'for-rent': 'rent', 'new-launch': 'new', 'sold': 'sold', 'rented': 'rented' };
  ...
  return `
    <div class="prop-card" onclick="window.open('${p.propUrl || '/property-detail?id=' + p._id}','_blank')" style="cursor:pointer">
      <div class="card-img" id="${id}">
        ...
        ${(p.badgeClass === 'sold' || p.badgeClass === 'rented') ? '<div class="card-sold-overlay"><div class="card-sold-stamp ' + (p.badgeClass === 'sold' ? 'is-sold' : 'is-rented') + '"><i class="fa-solid fa-ban"></i> ' + (p.badgeClass === 'sold' ? 'Sold Out' : 'Rented Out') + '</div></div>' : ''}
      </div>
      <div class="card-body">
  ...
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      fetch(API + '/properties?limit=1')
        ...
        .catch(() => {
          ['dynamicPropCount','ctaPropCount','authPropCount'].forEach(id => {
